...
gjr111_preds = pd.Series(forecasts, index=test_idx)

# Reset forecasts
forecasts = []

# Repeat process for the GJR-GARCH(2,1,2) model
for i in range(len(test_idx)):
    ...
    # Train GJR-GARCH(2,1,2) model
    model = arch_model(train, vol='GARCH', p=2, o=1, q=2, dist = 'studentst') 
    model_fit = model.fit(disp='off')
...
gjr212_preds = pd.Series(forecasts, index=test_idx)
...
plt.plot(gjr111_preds, color = "black", label="GJR-GARCH(1,1,1)",  alpha=0.75)
plt.plot(gjr212_preds, color = "blue", label="GJR-GARCH(2,1,2)",  alpha=0.75)
plt.legend(loc="upper right")
plt.title('GJR-GARCH Model Expanding Window Forecasts')
plt.show()
...
gjr111_rmse = math.sqrt(np.square(np.subtract(gjr111_preds, y_test)).mean())
gjr212_rmse = math.sqrt(np.square(np.subtract(gjr212_preds, y_test)).mean())

gjr111_mae = mean_absolute_error(gjr111_preds, y_test)
gjr212_mae = mean_absolute_error(gjr212_preds, y_test)


print("GJR-GARCH(1,1,1) Root Mean Square Error:")
print(gjr111_rmse)
print()

print("GJR-GARCH(2,1,2) Root Mean Square Error:")
print(gjr212_rmse)
print()

print("GJR-GARCH(1,1,1) Mean Absolute Error:")
print(gjr111_mae)
print()

print("GJR-GARCH(2,1,2) Mean Absolute Error:")
print(gjr212_mae)
print()
